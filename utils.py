...
def data_split(repe_time):  #划分数据集，训练集：验证集：测试集=6：2：2
    with open('./lookupcsv/ADNI.csv', 'r') as f: #读取ADNI.csv文件
        reader = csv.reader(f)
        your_list = list(reader)
    labels, train_valid, test = your_list[0:1], your_list[1:636], your_list[636:] #按照训练集：验证集：测试集=6：2：2划分
    for i in range(repe_time): #划分repe_time次
        random.shuffle(train_valid)
        folder = 'lookupcsv/exp{}/'.format(i) #每次划分得到的训练集、验证集、测试集放到一个exp文件夹中
        if not os.path.exists(folder):
            os.mkdir(folder) 
        with open(folder + 'train.csv', 'w', newline='') as myfile:
            wr = csv.writer(myfile, quoting=csv.QUOTE_ALL)
            wr.writerows(labels + train_valid[:476])
